    ...
    try {
        ...
        if (result.length > 0) {
            ...
            const borne = await Borne.findOne({ where: { idBorne: req.body.idBorne } })
                // update borne params 
            const update = await Borne.update({ nbPlaces: borne.nbPlaces - 1 }, { where: { idBorne: req.body.idBorne } })
            ...
            if (vehicule) { // Check if record exists in db
                ...
                if (updatedVehicule) {
                    ...
                    if (idAnienneBorne != req.body.idBorne) {
                        ...
                        let updateAncienneBorne = await Borne.update({ nbPlaces: ancienneBorne.nbPlaces + 1 }, { where: { idBorne: idAnienneBorne } })
                        ...
                        let updateNouvelleBorne = await Borne.update({ nbPlaces: nouvelleBorne.nbPlaces - 1 }, { where: { idBorne: req.body.idBorne } })
...
const updateEtatVehicule = async(req, res) => {
    ...
    if (!req.body.pressionHuileMoteur || !req.body.chargeBatterie || !req.body.pressionPneus) {
        res.status(400).send({
            message: "Content can not be empty!"
        });
        return;
    ...
    try {
        await Vehicule.update({
            pressionHuileMoteur: req.body.pressionPneus,
            chargeBatterie: req.body.chargeBatterie,
            pressionPneus: req.body.pressionPneus
        }, {
            where: {
        ...
        res.status(500).send({
            error: err.message || "Some error occurred while updating the vehicule."
        });
    }
}
...
const getVehiculeReservations = async(req, res, next) => {
    ...
    try {
        ...
        res.status(500).send({
            error: err.message || 'Some error occured while retreiving vehicule"s reservation history',
        });
    }
};
    ...
    try {
        ...
        if (state) {
            ...
            if (state === 'en service' || state === 'hors service') {
                const response = await Vehicule.update({ etat: state }, {
                    returning: true,
                    where: {
                ...
                res.status(400).send({
                    message: "Attribute 'etat' must be either 'en service' or 'hors service'",
                });
                return;
            }
        } else {
